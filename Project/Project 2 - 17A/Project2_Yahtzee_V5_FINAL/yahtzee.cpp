 ...
 * Created player header file which will store player info
 * Reconstructed 12/16 header files
 */
#include "Yahtzee.h"
...
void Yahtzee::play() 
{
    ...
    do 
    {  
      //For multiple rounds, start over here.
...
bool Yahtzee::load() 
{
    ...
    int pc = 0;
    In >> round >> pc;  
    // loads the current round and player count
    ...
    for( int i = 0; i < pc; i++ ) 
    {
        ...
        In >> name >> score; 
         // loads player information such as Name and Score
        if (name[0] != '_')
        {

          players.push_back(new HumanPlayer(name)); 
           // pushes a player into Players
...
void Yahtzee::setup_players() 
{
    ...
    do 
    {
        ...
        if( pc < 1 || pc > 4 ) 
        {
            cout << "You must enter a number from 1 to 4 (names that start with '_' will be considered CPU Players)" << endl;
        }
    } 
    while( pc < 1 || pc > 4 );
...
int Yahtzee::player_turn(int index) 
{
    ...
    bool r2ndT, r3rdT;                      
    //Rolled 2nd Time? Rolled 3rd time?

    //Will player roll a 2nd or 3rd time?
    char ans;
    string s;
    int scr3k;             
     //Score with 3 of a Kind

    cout << "\n===================================================\n"
         << "It's " << 
         players[index]->getName() << "'s turn:" << endl;
    ...
    cout << *players[index] << endl;
       cout << endl;    
        ...
        ans = toupper( players[index]->get_input() ); 
        //will make sure entry is uppercase

        if( ans == 'R' ) 
        {  
          //User wants to reroll
            r2ndT = true;
            ...
            for( int i = 0; i < s.length(); i++ )
             {      
              ...
              //String property -- treating as array
            ...
            {  
              //User wants to reroll
                r3rdT = true;   
                //this was rolled 2nd time. was it a mistake to change this to 3rd?
                ...
                std::getline( cin, s );  
                //Get whatever user types ('a' - 'e' hopefully)
                ...
                {      
                  char choice = s[i];  
                  //This is a property strings have -- treat as array
...
void Yahtzee::save_game() 
{
  ...
  Out << round << " " << players.size() << endl;  
  // Saves the current round and amount of players
  for( int i = 0; i < players.size(); i++ ) 
  {

    Out << players[i]->getName() << " " << scores[i] << endl; 
  ...
  sve_cnt++;  
  // Increments how many times the game has been saved in this instance
  // of yahtzee
}
